...
describe('useCalendarView', () => {
  ...
  it("주간 뷰에서 다음으로 navigate시 7일 후 '2025-10-08' 날짜로 지정이 된다", () => {
    ...
    expect(result.current.currentDate.toDateString()).toBe(new Date('2025-10-08').toDateString());
  });
  ...
  it("주간 뷰에서 이전으로 navigate시 7일 전 '2025-09-24' 날짜로 지정이 된다", () => {
    ...
    expect(result.current.currentDate.toDateString()).toBe(new Date('2025-09-24').toDateString());
  });
  ...
  it("월간 뷰에서 다음으로 navigate시 한 달 후 '2025-11-01' 날짜여야 한다", () => {
    ...
    expect(result.current.currentDate.toDateString()).toBe(new Date('2025-11-01').toDateString());
  });
  ...
  it("월간 뷰에서 이전으로 navigate시 한 달 전 '2025-09-01' 날짜여야 한다", () => {
    ...
    expect(result.current.currentDate.toDateString()).toBe(new Date('2025-09-01').toDateString());
  });

  it("currentDate가 '2025-03-01'로 변경되면 3월 휴일 '삼일절'로 업데이트되어야 한다", () => {
    const { result } = renderHook(() => useCalendarView());
    act(() => {
