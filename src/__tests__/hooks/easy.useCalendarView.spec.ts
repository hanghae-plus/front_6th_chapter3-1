...
describe('useCalendarView Hook Test', () => {
  ...
  describe('연도 경계 테스트', () => {
    it('2025년 1월에서 이전으로 이동 시 2024년 12월로 바뀌어야 한다', () => {
      const { result } = renderHook(() => useCalendarView());

      act(() => {
        result.current.setCurrentDate(new Date('2025-01-05'));
      });
    ...
    it('2024년 12월에서 다음으로 이동 시 2025년 1월로 바뀌어야 한다', () => {
      const { result } = renderHook(() => useCalendarView());

      act(() => {
        result.current.setCurrentDate(new Date('2024-12-01'));
      });
  ...
  describe('주간 뷰 경계 테스트', () => {
    it('2024년 12월 마지막 주에서 다음으로 이동 시 2025년 1월 첫째 주로 바뀌어야 한다', () => {
      const { result } = renderHook(() => useCalendarView());
      ...
      act(() => {
        result.current.setCurrentDate(new Date('2024-12-29'));
      });
      ...
      assertDate(result.current.currentDate, new Date('2025-01-05'));
    });

    it('2025년 1월 첫째 주에서 이전으로 이동 시 2024년 12월 마지막 주로 바뀌어야 한다', () => {
      const { result } = renderHook(() => useCalendarView());
      ...
      assertDate(result.current.currentDate, new Date('2024-12-29'));
    });
  });
});
