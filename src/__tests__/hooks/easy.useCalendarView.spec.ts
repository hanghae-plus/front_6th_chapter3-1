...
describe('초기 상태', () => {
  ...
  it('view는 "month"이어야 한다', () => {
    ...
    expect(initialState.view).toBe('month');
  });
  ...
  it('currentDate는 오늘 날짜인 "2025-10-01"이어야 한다', () => {
    ...
    assertDate(initialState.currentDate, new Date('2025-10-01T00:00:00'));
  });
...
it("view를 'week'으로 변경 시 적절하게 반영된다", () => {
  ...
  /** state변경이나 DOM 업데이트 등을 할 땐 act로 감싸줘야 함 */
  act(() => {
...
it("주간 뷰에서 다음으로 navigate시 7일 후 '2025-10-08' 날짜로 지정이 된다", () => {
  ...
  assertDate(result.current.currentDate, new Date('2025-10-08T00:00:00Z'));
...
it("주간 뷰에서 이전으로 navigate시 7일 전 '2025-09-24' 날짜로 지정이 된다", () => {
  ...
  assertDate(result.current.currentDate, new Date('2025-09-24T00:00:00Z'));
...
it("월간 뷰에서 다음으로 navigate시 한 달 후 '2025-11-01' 날짜여야 한다", () => {
  ...
  assertDate(result.current.currentDate, new Date('2025-11-01T00:00:00Z'));
...
it("월간 뷰에서 이전으로 navigate시 한 달 전 '2025-09-01' 날짜여야 한다", () => {
  ...
  assertDate(result.current.currentDate, new Date('2025-09-01T00:00:00Z'));
...
it("currentDate가 '2025-03-01' 변경되면 3월 휴일 '삼일절'로 업데이트되어야 한다", async () => {
  ...
  act(() => {
    result.current.setCurrentDate(new Date('2025-03-01T00:00:00Z'));
  });
