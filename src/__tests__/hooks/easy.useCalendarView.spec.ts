...
describe('useCalendarView', () => {
  ...
  describe('초기 상태', () => {
    ...
    it('holidays는 10월 휴일인 개천절, 한글날, 추석이 지정되어 있어야 한다', () => {
      ...
      expect(result.current.holidays).toHaveProperty('2025-10-06', '추석');
      expect(result.current.holidays).toHaveProperty('2025-10-07', '추석');
  ...
  it("currentDate가 '2025-03-01' 변경되면 3월 휴일 '삼일절'로 업데이트되어야 한다", async () => {
    ...
    expect(result.current.holidays).not.toHaveProperty('2025-10-03');
    expect(result.current.holidays).not.toHaveProperty('2025-10-09');
