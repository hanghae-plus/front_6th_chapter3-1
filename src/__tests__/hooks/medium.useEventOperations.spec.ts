...
it("새로 정의된 'title', 'endTime' 기준으로 적절하게 일정이 업데이트 된다", async () => {
  ...
  expect(result.current.events[1].title).toBe('기존 회의 14:00으로 수정');
  expect(result.current.events[1].endTime).toBe('14:00');
});
...
it("네트워크 오류 시 '일정 삭제 실패'라는 텍스트가 노출되며 이벤트 삭제가 실패해야 한다", async () => {
  ...
  await act(async () => {
    await result.current.deleteEvent('15');
  });
