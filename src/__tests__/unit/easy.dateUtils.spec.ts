...
describe('getEventsForDay', () => {
  ...
  it('날짜가 32일 이상인 경우 빈 배열을 반환한다', () => {
    ...
    const events2: Event[] = [];
    const date2 = new Date('2025-08-33');
    const result2 = getEventsForDay(events2, date2.getDate());
...
describe('formatWeek', () => {
  ...
  it('월의 마지막 주에 대해 올바른 주 정보를 반환한다', () => {
    ...
    const result = formatWeek(date);
    expect(result).toEqual('2025년 9월 1주');
  });
...
describe('isDateInRange', () => {
  ...
  it('시작일이 종료일보다 늦은 경우 모든 날짜에 대해 false를 반환한다', () => {
    const date = new Date('2025-07-01');
    const result = isDateInRange(date, new Date('2025-07-31'), new Date('2025-07-01'));
    expect(result).toBe(false);
  });
});
...
describe('formatDate', () => {
  ...
  it('월이 한 자리 수일 때 앞에 0을 붙여 포맷팅한다', () => {
    const date = new Date('2025-8-01');
    const result = formatDate(date);
    expect(result).toBe('2025-08-01');
  });

  it('일이 한 자리 수일 때 앞에 0을 붙여 포맷팅한다', () => {
    const date = new Date('2025-08-5');
    const result = formatDate(date);
