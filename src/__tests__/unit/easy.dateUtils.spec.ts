...
describe('getEventsForDay', () => {
  ...
  it('해당 날짜에 이벤트가 없을 경우 빈 배열을 반환한다', () => {
    const result = getEventsForDay(mockEvents, 22);
    // 배열 길이가 0개인지 확인 (0개 맞음)
...
describe('formatWeek', () => {
  it('월의 중간 날짜(2024년 7월 15일 / 2024년 7월 3주)에 대해 올바른 주 정보를 반환한다', () => {
    expect(formatWeek(new Date('2024-07-15'))).toBe('2024년 7월 3주');
  });

  it('월의 첫 주(2024년 7월 1일 / 2024년 7월 1주)에 대해 올바른 주 정보를 반환한다', () => {
    expect(formatWeek(new Date('2024-07-01'))).toBe('2024년 7월 1주');
  });

  it('월의 마지막 주(2024년 1월 31일 / 2024년 2월 1주)에 대해 올바른 주 정보를 반환한다', () => {
    expect(formatWeek(new Date('2024-01-031'))).toBe('2024년 2월 1주');
  });

  it('연도가 바뀌는 주(2024년 12월 31일 / 2025년 1월 1주)에 대해 올바른 주 정보를 반환한다', () => {
    expect(formatWeek(new Date('2024-12-31'))).toBe('2025년 1월 1주');
  });

  it('윤년 2월의 마지막 주(2024년 2월 29일 / 2024년 2월 5주)에 대해 올바른 주 정보를 반환한다', () => {
    expect(formatWeek(new Date('2024-02-29'))).toBe('2024년 2월 5주');
  });

  it('평년 2월의 마지막 주(2100년 2월 28일 / 2100년 3월 1주)에 대해 올바른 주 정보를 반환한다', () => {
    expect(formatWeek(new Date('2100-02-28'))).toBe('2100년 3월 1주');
  });
});
...
describe('fillZero', () => {
  ...
  it('value가 지정된 size보다 큰 자릿수를 가지면 원래 값을 그대로 반환한다', () => {
    expect(fillZero(1000, 1)).toBe('1000');
  });
});
