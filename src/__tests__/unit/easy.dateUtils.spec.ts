...
import { DINNER_0901, LUNCH_0901, LUNCH_0904 } from '../mockEvents';
...
describe('getWeekDates', () => {
  it('주중의 날짜(수요일)에 대해 올바른 주의 날짜들을 반환한다', () => {
    //일요일 부터 시작
    expect(getWeekDates(new Date('2025-08-22'))).toEqual([
      new Date('2025-08-17'),
      new Date('2025-08-18'),
      new Date('2025-08-19'),
      new Date('2025-08-20'),
  ...
  it('주의 시작(월요일)에 대해 올바른 주의 날짜들을 반환한다', () => {
    expect(getWeekDates(new Date('2025-07-28'))).toEqual([
      new Date('2025-07-27'),
      new Date('2025-07-28'),
      new Date('2025-07-29'),
      new Date('2025-07-30'),
  ...
  it('윤년의 2월 29일을 포함한 주를 올바르게 처리한다', () => {
    expect(getWeekDates(new Date('2000-02-29'))).toEqual([
      new Date('2000-02-27'),
      new Date('2000-02-28'),
      new Date('2000-02-29'),
      new Date('2000-03-01'),
...
describe('getWeeksAtMonth', () => {
  it('2025년 7월 1일의 올바른 주 정보를 반환해야 한다', () => {
    expect(getWeeksAtMonth(new Date('2025-07-01'))[0]).toEqual([null, null, 1, 2, 3, 4, 5]);
  });
});

describe('getEventsForDay', () => {
  const events: Event[] = [LUNCH_0901, DINNER_0901, LUNCH_0904];
  it('특정 날짜(1일)에 해당하는 이벤트만 정확히 반환한다', () => {
    expect(getEventsForDay(events, 1)).toEqual([LUNCH_0901, DINNER_0901]);
  });
...
describe('formatWeek', () => {
  ...
  it('월의 첫 주에 대해 올바른 주 정보를 반환한다', () => {
    expect(formatWeek(new Date('2025-08-04'))).toBe('2025년 8월 1주');
  });
...
describe('isDateInRange', () => {
  ...
  it('범위의 시작일 2025-07-01에 대해 true를 반환한다', () => {
    expect(isDateInRange(new Date('2025-07-01'), startDate, endDate)).toBe(true);
  });

  it('범위의 종료일 2025-07-31에 대해 true를 반환한다', () => {
    expect(isDateInRange(new Date('2025-07-31'), startDate, endDate)).toBe(true);
  });
  ...
  it('시작일이 종료일보다 늦은 경우 모든 날짜에 대해 false를 반환한다', () => {
    expect(
      isDateInRange(new Date('2025-07-01'), new Date('2025-08-01'), new Date('2025-07-01'))
    ).toBe(false);
    expect(
      isDateInRange(new Date('2025-08-01'), new Date('2025-08-01'), new Date('2025-07-01'))
    ).toBe(false);
    expect(
      isDateInRange(new Date('2025-07-21'), new Date('2025-08-01'), new Date('2025-07-01'))
    ).toBe(false);
    expect(
      isDateInRange(new Date('2025-09-21'), new Date('2025-08-01'), new Date('2025-07-01'))
    ).toBe(false);
  });
});
...
describe('formatDate', () => {
  it('날짜를 YYYY-MM-DD 형식으로 포맷팅한다', () => {
    expect(formatDate(new Date())).toBe('2025-10-01');
  });

  it('day 파라미터가 제공되면 해당 일자로 포맷팅한다', () => {
    expect(formatDate(new Date(), 4)).toBe('2025-10-04');
  });
