...
describe('getDaysInMonth', () => {
  ...
  it('0월 이전 해의 12월의 일수를 반환한다', () => {
    expect(getDaysInMonth(2025, 0)).toBe(31);
  });

  it('13월은 다음 해의 1월의 일수를 반환한다', () => {
...
describe('isDateInRange', () => {
  ...
  it('시작일이 종료일보다 늦은 경우 모든 날짜에 대해 false를 반환한다', () => {
    ...
    expect(isDateInRange(new Date('2025-08-02'), startDate, endDate)).toBe(false);
    expect(isDateInRange(new Date('2025-08-03'), startDate, endDate)).toBe(false);
  });
});
...
describe('formatDate', () => {
  it('날짜를 YYYY-MM-DD 형식으로 포맷팅한다', () => {
    expect(formatDate(new Date('2025-08-15'))).toBe('2025-08-15');
  });

  it('day 파라미터가 제공되면 해당 일자로 포맷팅한다', () => {
    expect(formatDate(new Date('2025-08-15'), 10)).toBe('2025-08-10');
  });
  ...
  it('일이 한 자리 수일 때 앞에 0을 붙여 포맷팅한다', () => {
    expect(formatDate(new Date('2025-08-05'))).toBe('2025-08-05');
  });
});
