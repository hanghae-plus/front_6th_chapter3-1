...
describe('getDaysInMonth', () => {
  ...
  it('0월을 입력하면 이전 해 12월의 일수를 반환한다', () => {
    expect(getDaysInMonth(2025, 0)).toBe(31);
  });

  it('13월을 입력하면 다음 해 1월의 일수를 반환한다', () => {
...
describe('getWeekDates', () => {
  it('주중의 날짜(수요일)에 대해 올바른 주의 날짜들을 반환한다', () => {
    ...
    const days = getWeekDates(new Date(date)).map((d) => d.getDate());
  ...
  it('연도를 넘어가는 주의 날짜를 정확히 반환한다 (연말)', () => {
    const date = '2024-12-31';
    const days = getWeekDates(new Date(date)).map((d) => formatDate(d));
    expect(days).toEqual([
      '2024-12-29',
      '2024-12-30',
      '2024-12-31',
  ...
  it('연도를 넘어가는 주의 날짜를 정확히 반환한다 (연초)', () => {
  ...
  it('윤년의 2월 29일을 포함한 주의 날짜를 정확히 반환한다', () => {
    const date = '2024-02-29';
    const days = getWeekDates(new Date(date)).map((d) => d.getDate());
    expect(days).toEqual([25, 26, 27, 28, 29, 1, 2]);
  });

  it('월의 마지막 날짜를 포함한 주의 날짜를 정확히 반환한다', () => {
    const date = '2025-08-31';
    const days = getWeekDates(new Date(date)).map((d) => d.getDate());
    expect(days).toEqual([31, 1, 2, 3, 4, 5, 6]);
  });
});

describe('getWeeksAtMonth', () => {
  it('2025년 7월 1일이 속한 월의 날짜들을 정확히 반환해야 한다', () => {
    const date = '2025-07-01';
    const days = getWeeksAtMonth(new Date(date));
    expect(days.length).toBe(5);
    expect(days[0]).toEqual([null, null, 1, 2, 3, 4, 5]);
    expect(days[4]).toEqual([27, 28, 29, 30, 31, null, null]);
  });
});
...
describe('formatWeek', () => {
  it('월의 중간 날짜에 대해 올바른 주 정보를 반환한다', () => {
    expect(formatWeek(new Date('2025-07-15'))).toBe('2025년 7월 3주');
  });
...
describe('isDateInRange', () => {
  ...
  it('범위 내의 날짜 2025-07-10에 대해 true를 반환한다', () => {
    expect(isDateInRange(new Date('2025-07-10'), start, end)).toBe(true);
  });
  ...
  it('시작일이 종료일보다 늦은 경우 모든 날짜에 대해 false를 반환한다', () => {
    expect(isDateInRange(new Date('2025-07-15'), end, start)).toBe(false);
  });
});
...
describe('formatDate', () => {
  it('날짜를 YYYY-MM-DD 형식으로 포맷팅한다', () => {
    expect(formatDate(new Date('2025-08-19'))).toBe('2025-08-19');
  });

  it('day 파라미터가 제공되면 해당 일자로 포맷팅한다', () => {
    expect(formatDate(new Date('2025-08-19'), 5)).toBe('2025-08-05');
  });

  it('월이 한 자리 수일 때 앞에 0을 붙여 포맷팅한다', () => {
    expect(formatDate(new Date('2025-01-10'))).toBe('2025-01-10');
  });

  it('일이 한 자리 수일 때 앞에 0을 붙여 포맷팅한다', () => {
    expect(formatDate(new Date('2025-10-01'))).toBe('2025-10-01');
  });
});
