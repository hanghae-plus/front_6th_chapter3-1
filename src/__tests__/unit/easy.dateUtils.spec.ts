...
describe('getWeeksAtMonth', () => {
  it('2025년 7월 1일의 올바른 주 정보를 반환해야 한다', () => {
    ...
    expect(weeks[0].slice(0, 2).every(d => d === null)).toBe(true);
...
describe('getEventsForDay', () => {
  ...
  it('특정 날짜(1일)에 해당하는 이벤트만 정확히 반환한다', () => {
    expect(getEventsForDay(events, 1)).toHaveLength(2);
    expect(getEventsForDay(events, 1).map(e => e.id)).toEqual(['1', '2']);
  });
...
describe('formatWeek', () => {
  ...
  it('월의 마지막 주에 대해 올바른 주 정보를 반환한다', () => {
    ...
    expect(formatWeek(date)).toBe('2025년 9월 1주');
  });
...
describe('formatDate', () => {
  ...
  it('day 파라미터가 제공되면 해당 일자로 포맷팅한다', () => {
    const date = new Date('2025-08-01');
    expect(formatDate(date, 15)).toBe('2025-08-15');
  });
