...
describe('parseDateTime', () => {
  ...
  it('잘못된 시간 형식에 대해 Invalid Date를 반환한다', () => {
    expect(parseDateTime('2025-07-01', '24:30')).toEqual(new Date('Invalid Date'));
  });
...
describe('convertEventToDateRange', () => {
  it('일반적인 이벤트를 올바른 시작 및 종료 시간을 가진 객체로 변환한다', () => {
    const event = createEvent({ id: '1', date: '2025-08-18' });

    expect(convertEventToDateRange(event)).toEqual({
      start: new Date('2025-08-18T09:00'),
      end: new Date('2025-08-18T10:00'),
    });
  });
...
describe('isOverlapping', () => {
  it('두 이벤트가 겹치는 경우 true를 반환한다', () => {
    const event1 = createEvent({ id: '1', date: '2025-08-18' });
    const event2 = createEvent({ id: '2', date: '2025-08-18' });
  ...
  it('두 이벤트가 겹치지 않는 경우 false를 반환한다', () => {
    const event1 = createEvent({ id: '1', date: '2025-08-18' });
    const event2 = createEvent({ id: '2', date: '2025-08-28' });
